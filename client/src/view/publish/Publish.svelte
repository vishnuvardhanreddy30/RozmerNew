<script>
    ...
    function ongetPostSuccess(res) {
        userFeedCount = res['length'];

        setTimeout(() => {
            if(userFeedCount === 1) {
                Base.toast('success', Labels.alert.one_feed_away);
            } else if(userFeedCount === 2) {
                Base.toast('success', Labels.alert.life_time_free);
            }
        }, 2000);
    }
...
<div class="flex-cont flex-dir-column wh-100-percent publish-cont">
    {#if userFeedCount < 2} 
        <div class="publish-free-art-alert">*{Labels.alert.post_2_feed}</div>
    {/if}
    <Toolbar ui="plain">
        <TextField
